...
export function Dashboard() {
  ...
  return (
    <div className="p-6 md:p-10 max-w-[1400px] mx-auto min-h-screen relative font-sans overflow-hidden bg-[#eef2f6]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8 relative z-10 items-center">
          ...
          >
             ...
             <div className="absolute inset-8 md:inset-10 z-20 pointer-events-none drop-shadow-[0_0_25px_rgba(16,185,129,0.4)] transition-transform duration-700">
                <svg className="w-full h-full transform -rotate-90">
                   <circle cx="50%" cy="50%" r="42%" stroke="#cbd5e1" strokeWidth="24" fill="none" opacity="0.2" strokeLinecap="round" />
                   <motion.circle 
                     cx="50%" cy="50%" r="42%" 
             ...
             <div className="text-center z-40 flex flex-col items-center">
               <div className="flex items-start justify-center mt-2">
                  <motion.span 
                    key="score-val"
                    className="text-[5rem] md:text-[6.5rem] font-bold text-slate-700 tracking-tighter leading-none"
                    style={{ textShadow: '2px 2px 4px rgba(163,177,198,0.3)' }}
                  >
                    {score}
                  </motion.span>
                  <span className="text-3xl font-bold text-slate-500 mt-3">%</span>
               </div>
               <p className="text-slate-500 font-semibold text-sm mt-1 uppercase tracking-widest flex items-center gap-1">
                 Business Health <ArrowUp className="w-4 h-4 text-emerald-500" strokeWidth={3} />
               </p>
             </div>
          </motion.div>
        </div>
      ...
      <motion.div 
        ...
        className="relative overflow-hidden bg-[#eef2f6] rounded-[2rem] p-6 md:p-8 shadow-[12px_12px_32px_rgba(163,177,198,0.4),-12px_-12px_32px_rgba(255,255,255,1)] border border-white/60 z-10"
      >
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-6 relative z-10">
        <h4 className="text-[1.1rem] font-bold text-slate-600 drop-shadow-sm">{title}</h4>
        {topRightIcon === 'arrow' ? (
          <div className="w-8 h-8 rounded-full bg-[#eef2f6] shadow-[3px_3px_6px_rgba(163,177,198,0.3),-3px_-3px_6px_rgba(255,255,255,0.9)] flex items-center justify-center">
            <ArrowUp className="w-4 h-4 text-emerald-500" strokeWidth={3} />
          </div>
        ) : (
          <div className="relative">
            <button 
              className="w-8 h-8 rounded-full bg-[#eef2f6] shadow-[3px_3px_6px_rgba(163,177,198,0.3),-3px_-3px_6px_rgba(255,255,255,0.9)] flex items-center justify-center text-slate-400 hover:text-emerald-500 transition-colors active:shadow-[inset_2px_2px_4px_rgba(163,177,198,0.3),inset_-2px_-2px_4px_rgba(255,255,255,0.9)]"
            ...
            <AnimatePresence>
              {menuOpen && (
                <motion.div 
                  ...
                  className="absolute right-0 top-10 w-36 bg-[#eef2f6] shadow-[8px_8px_16px_rgba(163,177,198,0.4),-8px_-8px_16px_rgba(255,255,255,0.9)] border border-white/50 rounded-xl overflow-hidden z-50 flex flex-col p-1"
                >
                  <button className="px-3 py-2 text-xs text-left font-bold text-slate-600 hover:bg-slate-200/50 rounded-lg transition-colors" onClick={() => setMenuOpen(false)}>Options</button>
                  <button className="px-3 py-2 text-xs text-left font-bold text-slate-600 hover:bg-slate-200/50 rounded-lg transition-colors" onClick={() => setMenuOpen(false)}>Export</button>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
      ...
      <div className="flex items-center gap-4 mt-auto relative z-10 w-full">
        ...
        <div className="relative w-20 h-20 md:w-24 md:h-24 flex items-center justify-center shrink-0">
           ...
           <div className="absolute inset-2 rounded-full bg-[#eef2f6] shadow-[inset_4px_4px_8px_rgba(163,177,198,0.4),inset_-4px_-4px_8px_rgba(255,255,255,0.9)] z-0"></div>
           ...
           <svg className="absolute inset-0 w-full h-full transform -rotate-90 z-10 overflow-visible">
              <motion.circle 
                ...
                strokeLinecap="round" 
                style={{ filter: \`drop-shadow(0px 0px 6px \${circleColor}60)\` }} 
              />
           </svg>
           {/* Outer bevel ring */}
           <div className="absolute inset-[3px] rounded-full border border-white/60 z-20 pointer-events-none"></div>

           <motion.span 
             key={formattedPercentage}
             initial={{ opacity: 0.8, scale: 0.9 }}
             animate={{ opacity: 1, scale: 1 }}
             className="text-xl md:text-2xl font-bold text-slate-700 z-30 drop-shadow-sm"
           >
             {formattedPercentage}%
           </motion.span>
        </div>
        ...
        {/* Animated Line Chart */}
        <div className="flex-1 h-16 md:h-20 opacity-90 group-hover:opacity-100 transition-opacity ml-2">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data}>
              ...
              <Line 
                ...
                activeDot={{ r: 6, strokeWidth: 3, stroke: '#fff', fill: lineColor }}
                style={{ filter: \`drop-shadow(0px 6px 6px \${lineColor}40)\` }}
              />
            </LineChart>
          </ResponsiveContainer>
        </div>
