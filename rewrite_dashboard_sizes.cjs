...
export function Dashboard() {
  ...
  return (
    <div className="p-6 md:p-10 max-w-[1600px] mx-auto min-h-screen relative font-sans overflow-hidden bg-[#eef2f6]">
      ...
      {/* Header */}
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4 pl-4">
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-slate-700 tracking-tight flex items-center gap-3 drop-shadow-sm">
            BizManager
          </h1>
        </div>
      ...
      {/* Main Dashboard Layout - Proportional sizing */}
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-6 md:gap-12 mb-10 relative z-10 items-center justify-center">
        
        {/* Left Column Cards */}
        <div className="flex flex-col gap-6 md:gap-8 order-2 lg:order-1 w-full max-w-sm mx-auto lg:ml-auto lg:mr-0">
          <StatCard 
        ...
        <div className="flex justify-center items-center order-1 lg:order-2 py-4">
          <motion.div 
            ...
            className="relative w-[320px] h-[320px] md:w-[500px] md:h-[500px] flex items-center justify-center group mx-auto"
          >
             {/* Outer Thick Glass Ring (Neumorphic base) */}
             <div className="absolute inset-0 rounded-full bg-[#eef2f6] shadow-[16px_16px_40px_rgba(163,177,198,0.4),-16px_-16px_40px_rgba(255,255,255,1)] z-0"></div>
             
             <div className="absolute inset-3 md:inset-5 rounded-full bg-transparent border-2 border-white/50 shadow-[inset_8px_8px_16px_rgba(163,177,198,0.3),inset_-8px_-8px_16px_rgba(255,255,255,0.8)] z-10"></div>

             {/* The Glowing Gauge Arc */}
             <div className="absolute inset-10 md:inset-14 z-20 pointer-events-none drop-shadow-[0_0_30px_rgba(16,185,129,0.5)] transition-transform duration-700">
                <svg className="w-full h-full transform -rotate-90">
                   <circle cx="50%" cy="50%" r="42%" stroke="#cbd5e1" strokeWidth="32" fill="none" opacity="0.25" strokeLinecap="round" />
                   <motion.circle 
                     cx="50%" cy="50%" r="42%" 
             ...
             {/* Inner Glass Dome */}
             <div className="absolute inset-20 md:inset-[5.5rem] rounded-full bg-[#eef2f6] shadow-[12px_12px_28px_rgba(163,177,198,0.4),-12px_-12px_28px_rgba(255,255,255,0.9),inset_4px_4px_8px_rgba(255,255,255,0.8)] z-30 border border-white/80"></div>
             
             {/* Center Content */}
             <div className="text-center z-40 flex flex-col items-center">
               <div className="flex items-start justify-center mt-2 md:mt-4">
                  <motion.span 
                    key="score-val"
                    className="text-[6rem] md:text-[8rem] font-bold text-slate-700 tracking-tighter leading-none"
                    style={{ textShadow: '2px 3px 6px rgba(163,177,198,0.4)' }}
                  >
                    {score}
                  </motion.span>
                  <span className="text-4xl md:text-5xl font-bold text-slate-500 mt-3 md:mt-6">%</span>
               </div>
               <p className="text-slate-500 font-semibold text-sm md:text-base mt-2 md:mt-4 uppercase tracking-widest flex items-center gap-1">
                 Business Health Score <ArrowUp className="w-5 h-5 md:w-6 md:h-6 text-emerald-500" strokeWidth={3} />
               </p>
             </div>
          </motion.div>
        </div>
        ...
        {/* Right Column Cards */}
        <div className="flex flex-col gap-6 md:gap-8 order-3 w-full max-w-sm mx-auto lg:mr-auto lg:ml-0">
          <StatCard 
            title="Cash Health" 
      ...
      <motion.div 
        ...
        className="relative overflow-hidden bg-[#eef2f6] rounded-[2rem] p-6 md:p-8 shadow-[12px_12px_32px_rgba(163,177,198,0.4),-12px_-12px_32px_rgba(255,255,255,1)] border border-white/60 z-10 mx-auto max-w-5xl"
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-12 relative z-10 pb-2">
           ...
           <div className="flex flex-col gap-1">
              ...
              <span className="text-2xl font-bold text-slate-700">714 6.061+</span>
           </div>
           
           <div className="flex flex-col gap-1">
              <span className="text-sm font-semibold text-slate-500 uppercase tracking-wide">Performance charged</span>
              <span className="text-2xl font-bold text-emerald-500 drop-shadow-sm">+3.91.3%</span>
           </div>
           
           <div className="flex flex-col gap-1">
              <span className="text-sm font-semibold text-slate-500 uppercase tracking-wide">Market Offices</span>
              <span className="text-2xl font-bold text-slate-700">120.004</span>
           </div>
           
           <div className="flex flex-col gap-1">
              <span className="text-sm font-semibold text-slate-500 uppercase tracking-wide">Def Rate</span>
              <span className="text-2xl font-bold text-slate-700">$3,65.800</span>
           </div>
        </div>
        
        {/* Decorative Sparkle */}
        <Sparkles className="absolute right-4 bottom-4 w-12 h-12 text-white/40 drop-shadow-md pointer-events-none transform -rotate-12" />
      </motion.div>
    </div>
  );
}
...
function StatCard({ title, percentage, circleColor, lineColor, data, topRightIcon = 'arrow' }: { title: string, percentage: number, circleColor: string, lineColor: string, data: any[], topRightIcon?: 'dots' | 'arrow' }) {
  ...
  return (
    <motion.div 
      ...
      className="p-4 md:p-5 flex-1 flex flex-col justify-between relative overflow-visible group bg-[#eef2f6] rounded-[1.5rem] shadow-[8px_8px_20px_rgba(163,177,198,0.4),-8px_-8px_20px_rgba(255,255,255,1)] border border-white/50 w-full"
    >
      <div className="flex justify-between items-start mb-5 relative z-10">
        <h4 className="text-[1.05rem] font-bold text-slate-600 drop-shadow-sm">{title}</h4>
        {topRightIcon === 'arrow' ? (
      ...
      <div className="flex items-center gap-3 mt-auto relative z-10 w-full">
        ...
        {/* Animated Line Chart */}
        <div className="flex-1 h-12 md:h-16 opacity-90 group-hover:opacity-100 transition-opacity ml-1">
